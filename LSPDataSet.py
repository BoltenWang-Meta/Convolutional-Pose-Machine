...
def guassian_kernel(center_x, center_y, sgm=3, size_w=47, size_h=47):
...
class LSPSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        Img, _ = normalize_ImgandLabel(img, label, 368)
        _, label = normalize_ImgandLabel(img, label, 47)
        img = Img
        center = np.array([[np.mean(label[0])], [np.mean(label[1])]])
        heat_map = generate_heatmap(label, (47, 47))
        center_map = generate_heatmap(center, (368, 368))
        ...
        return img, heat_map, center_map


def test():
    path = r"E:\9_A_PhD\DataSet\Leeds_Sport_Pose\DataSet"
    lspset = LSPSet(path, 'test')
